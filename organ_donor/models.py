...
class Donor(BaseModel):
    when_donated = models.DateTimeField(null=True, blank=True)


class Recipient(BaseModel):
    when_received = models.DateTimeField(null=True, blank=True)


class OrgPulseAdmin(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    is_valid = models.BooleanField(default=False)

    def __str__(self):
        return f"{self.user.username} - {self.is_valid}"
